...
interface ApiKeyModalProps {
    onClose: () => void;
    onSave: (keys: { gemini: string; openRouter: string }) => void;
    currentKeys: { gemini: string; openRouter: string };
}

type Tab = 'gemini' | 'fallback';

const ApiKeyModal: React.FC<ApiKeyModalProps> = ({ onClose, onSave, currentKeys }) => {
    ...
    const handleSave = () => {
        onSave({ gemini: geminiApiKey, openRouter: openRouterApiKey });
        onClose();
    };
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-md border border-gray-700">
                <div className="flex justify-between items-center p-4 border-b border-gray-700">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2"><KeyIcon/>Настройки API-ключей</h3>
                    <button onClick={onClose} className="text-gray-400 hover:text-white"><CloseIcon/></button>
                </div>
                <div className="px-6 pt-2 border-b border-gray-700">
                    <div className="flex items-center">
                        <TabButton tabId="gemini" label="Google Gemini" />
                        <TabButton tabId="fallback" label="Fallback-Сервисы" />
                    </div>
                </div>
