...
## 8. Исправление и стабилизация "Плана Б" (Локальная сборка)
-   **Где:** `services/ffmpegCommandBuilder.ts`, `services/packageService.ts`
-   **Проблема:** Скрипты, генерируемые для локальной сборки видео (`.bat`, `.sh`), содержали статичную и некорректную FFmpeg-команду. Она не учитывала динамическое количество входных файлов (например, звуковые эффекты) и неверно указывала финальный аудиопоток, что делало функцию неработоспособной для проектов со сложным саунд-дизайном.
-   **Решение:** Архитектура генерации команды была полностью переработана. Теперь `ffmpegCommandBuilder` формирует все части команды (список входных изображений, список входных SFX, сложный граф фильтров и карту финального аудиопотока) и возвращает их. Сервис `packageService` динамически собирает из этих частей **полностью корректную команду**, которая записывается в сборочные скрипты. Это гарантирует, что локальная сборка в точности воспроизводит проект со всеми его настройками.

---

## 7. Завершающие исправления и стабилизация

### 7.1. Восстановление функции ручного поиска SFX
-   **Где:** `services/ttsService.ts`
-   **Проблема:** При внедрении пакетной обработки SFX была случайно удалена функция `findSfxWithAi`, отвечающая за **одиночный** поиск. Это полностью сломало возможность подбирать и заменять звуковые эффекты вручную в редакторе подкаста.
-   **Решение:** Функция `findSfxWithAi` была восстановлена, что вернуло полную функциональность ручного управления звуковыми эффектами.

### 7.2. Исправление ошибки в "Плане Б" (Локальная сборка)
-   **Где:** `services/packageService.ts`
-   **Проблема:** Сервис для создания пакета локальной сборки не мог получить доступ к браузерному `AudioContext`, что приводило к ошибке при попытке рассчитать длительность аудио.
-   **Решение:** Добавлено безопасное обращение к `window.AudioContext`, что обеспечивает стабильную работу функции создания пакета.
